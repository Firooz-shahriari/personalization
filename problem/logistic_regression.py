##### this is copied from the dagp code and should be modified for the personalized dagp setup probably. 
...
class lr_l2(object):
    def __init__(
        ...
        if balanced == False:
            self.split_vec = np.sort(
                np.random.choice(np.arange(1, self.N), self.n - 1, replace=False)
            )
    ...
    def F_val(self, theta):  ##  objective function value at theta
        if self.balanced == True:
            f_val = (
                np.sum(
                    np.log(
                        np.exp(
                            np.multiply(-self.Y_train, np.matmul(self.X_train, theta))
                        )
                        + 1
                    )
                )
        ...
        if self.balanced == False:
            temp1 = np.log(
                np.exp(np.multiply(-self.Y_train, np.matmul(self.X_train, theta))) + 1
            )
    ...
    def localgrad(
        self, theta, idx, j=None
    ):  ## idx is the node index, j is local sample index
    ...
    def grad(self, theta, idx=None):  ## centralized stochastic/batch gradient
        if idx == None:  ## full batch
            ...
            if self.balanced == False:
                return (
                    np.sum(self.networkgrad(np.tile(theta, (self.n, 1))), axis=0)
                    / self.n
                )
        else:
            if self.balanced == True:
